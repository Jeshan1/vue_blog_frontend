<template>
      ...
      <div class="flex flex-wrap justify-between items-center gap-4 mx-24 w-full">
        ...
        <ul class="lg:flex lg:ml-14 lg:gap-x-5 max-lg:fixed max-lg:space-y-3 max-lg:bg-white max-lg:w-1/2 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50"
          ...
          <li v-for="item in menuItems" :key="item" class="max-lg:border-b max-lg:py-3 px-3">
            <router-link :to="item" class="lg:hover:text-[#007bff] text-black block text-[15px]">{{ item.toUpperCase() }}</router-link>
          </li>
        </ul>
  
        <!-- Mobile Menu Toggle Button -->
        <button @click="toggleMenu" class="lg:hidden fixed top-2 right-4 z-[100] rounded-full bg-white w-9 h-9 flex items-center justify-center border">
          <svg v-if="isMenuOpen" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 fill-black" viewBox="0 0 320.591 320.591">
        ...
        <!-- Search Bar -->
        <div class="">
          <div class="flex max-w-xl w-full bg-gray-100 px-4 py-2 rounded border focus-within:border-blue-600 focus-within:bg-transparent transition-all">
            <input v-model="searchQuery" @input="handleSearch" type="text" placeholder="Search something..." class="w-full text-sm bg-transparent rounded outline-none pr-2" />
  ...
  const handleSearch = () => {
    if (searchQuery.value) {
      router.push({ path: '/blogs', query: { search: searchQuery.value } });
    } else {
